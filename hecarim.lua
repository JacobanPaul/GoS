...
function LevelUp()
		...
		if "Hecarim" == GetObjectName(myHero)  then
...
if "Hecarim" == GetObjectName(myHero) then
	PrintChat("<font color='#1E90FF'>[<font color='#00BFFF'>GoS-U<font color='#1E90FF'>] <font color='#00BFFF'>Hecarim loaded successfully!")
	local HecarimMenu = Menu("[Hecarim", "Hecarim")
	HecarimMenu:Menu("Combo", "Combo")
	HecarimMenu.Combo:Slider('X','Minimum Enemies: R', 1, 0, 5, 1)
	local HecarimQ = { range = 0, radius = 350, speed = math.huge, delay = 0.3, type = "circular", collision = false, source = myHero}
	local HecarimR = { range = 1000, radius = 230,width = 350, speed = 1100, delay = 0.01, type = "line", hitbox=true, aoe=true, cc=true, collision = false, source = myHero}
 ...
 DrawCircle(pos,HecarimR.range,1,25,0xff4169e1)
...
function useR(target)
	if GetDistance(target) < HecarimR.range then
		
			local RPred = GetPredictionForPlayer(GetOrigin(myHero),target,GetMoveSpeed(target),HecarimR.speed,HecarimR.delay*1000,HecarimR.range,HecarimR.width,false,true)
	...
	if Mode() == "Combo" then
		...
		if GetItemSlot(myHero, 3142) >= 1 and ValidTarget(target, 1000) then
			...
			if CanUseSpell(myHero,_E) == READY   then
				if ValidTarget(target,  GetRange(myHero)+1100) then
					useE(target)
			...
			if CanUseSpell(myHero,_W) == READY then
				if ValidTarget(target,GetRange(myHero)+600) then
					useW(target)
			...
			if CanUseSpell(myHero,_R) == READY then
				if ValidTarget(target,HecarimR.range) then
					if EnemiesAround(myHero, HecarimR.range+GetRange(myHero)) >= HecarimMenu.Combo.X:Value() then
							useR(target)
					end
	...
	if Mode() == "LaneClear" then
		for _,mob in pairs(minionManager.objects) do
			if GetTeam(mob) == 300 then
				...
				if CanUseSpell(myHero,_W) == READY  then 
					if ValidTarget(mob, GetHitBox(myHero)+600) then
	...
	if Mode() == "LaneClear" then
		for _,minion in pairs(minionManager.objects) do
			if GetTeam(minion) == MINION_ENEMY then
				...
				if CanUseSpell(myHero,_Q) == READY then
					if ValidTarget(minion, GetRange(myHero)+75) then
